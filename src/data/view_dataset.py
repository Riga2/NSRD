...
def make_model(args, train=True):
    return ViewDataset(args, train)

class ViewDataset(BaseDataset):
    def __init__(self, args, train=True):
        super(ViewDataset, self).__init__(args)
        self.args = args
        self.train = train
        ...
        self.view_dirname = args.view_dirname

    def load_View_HR(self, folder_index, file_index, ext='.png'):
        filename = str(file_index) + ext
        hr_file_path = os.path.join(self.gt_dir, str(folder_index), self.view_dirname, filename)
        hr = data_utils.getFromExr(hr_file_path).astype(np.float32) / 255.0
        return hr

    def load_View_LR(self, folder_index, file_index, ext='.png'):
        filename = str(file_index) + ext
        lr_file_path = os.path.join(self.lr_dir, str(folder_index), self.view_dirname, filename)
        lr = data_utils.getFromExr(lr_file_path).astype(np.float32) / 255.0
        return lr
    ...
    def __getitem__(self, item):
        ...
        for index in range(file_index, file_index + self.num_frames_samples):
            # HR
            hr = self.load_View_HR(folder_index, index)
            ...
            for idx in range(index, index - self.number_previous_frames - 1, -1):
                file = self.load_View_LR(folder_index, idx)
                if self.useNormal:
